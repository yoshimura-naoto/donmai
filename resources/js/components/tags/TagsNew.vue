...
export default {
  ...
  methods: {
    ...
    getPosts(tagName) {
      ...
      axios.get('/api/post/tags/new/' + tagName + '?last_post_id=' + lastPostId)
        .then((res) => {
          // console.log(res.data);
          ...
          this.$nextTick(() => {
            ...
            if (bottomOfWindow && this.posts.length > 0 && this.posts.slice(-1)[0].id > res.data.lastPostId) this.getPosts(tagName);
          });
    ...
    editSubmit() {
      ...
      axios.post('/api/post/edit', data)
        .then((res) => {
          ...
          if (this.editPost.tags.includes(this.$route.params.name)) {
