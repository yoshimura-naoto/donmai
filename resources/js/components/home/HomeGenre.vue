      ...
      <div v-for="(post, index) in posts" :key="post.id">
        ...
        <div class="posts" v-if="post.user">
          ...
          <div class="input-area">
            ...
            <div class="post-name-menu" v-if="post.user && authUser.id">
              <!-- ユーザー名 -->
...
export default {
  ...
  methods: {
    ...
    getPosts(genreName) {
      ...
      axios.get('/api/post/genre/get/' + genreName + '?last_post_id=' + lastPostId)
        .then((res) => {
          // console.log(res.data);
          this.posts.push(...res.data.posts);
          if ((this.posts.length > 0 && this.posts.slice(-1)[0].id === res.data.lastPostId) || !res.data.lastPostId) {
            this.loadMorePosts = false;
          }
          this.postsLoading = false;
          this.$nextTick(() => {
            let bottomOfWindow = document.documentElement.scrollTop + window.innerHeight >= document.documentElement.offsetHeight;
            if (bottomOfWindow && this.posts.length > 0 && this.posts.slice(-1)[0].id > res.data.lastPostId) this.getPosts(genreName);
          });
        }).catch((error) => {
          console.log(error);
          this.postsLoading = false;
    ...
    editSubmit() {
      ...
      axios.post('/api/post/edit', data)
        .then((res) => {
          ...
          if (this.genres[this.editPost.genreIndex].route === this.$route.params.name) {
  ...
  mounted() {
    ...
    window.onscroll = () => {
      ...
      if (bottomOfWindow && !this.modalPostShow && !this.modalPostEditShow && !this.deletePostModalOpened && !this.modalImageShow) {
        this.getPosts(this.$route.params.name);
      }
    }
  },
